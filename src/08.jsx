...
const test1 = (input) => {
	...
	for (const [key, value] of Object.entries(antennas)) {
		for (let a = 0; a < value.length; a++) {
			for (let b = value.length - 1; b > a; b--) {
				...
				if (antinodeA[0] >= 0 && antinodeA[0] < xMax && antinodeA[1] >= 0 && antinodeA[1] < yMax) {
					antinodes.push(antinodeA);
				}
				if (antinodeB[0] >= 0 && antinodeB[0] < xMax && antinodeB[1] >= 0 && antinodeB[1] < yMax) {
	...
	const uniqueAntinodes = [...new Set(stringAntinodes)].map((p) => [parseInt(p.split('-')[0]), parseInt(p.split('-')[1])]);
		...
		for (let a = 0; a < value.length; a++) {
			for (let b = 0; b < value.length; b++) {
				if (a !== b) {
					...
					while (antinodeX >= 0 && antinodeX < xMax && antinodeY >= 0 && antinodeY < yMax) {
