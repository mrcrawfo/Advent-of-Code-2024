...
const reorder = (list, rules) => {
	for (let r = 0; r < rules.length; r++) {
		...
		const yIndex = list.indexOf(y);
		if (xIndex >= 0 && yIndex >= 0 && xIndex > yIndex) {
			const [t] = list.splice(xIndex, 1);
			list.splice(yIndex, 0, t);
		}
	}
	return list;
};

const sumMiddles = (list) => {
	let sum = 0;

	for (let c = 0; c < list.length; c++) {
		const update = list[c];
		const [middleNumber] = update.splice(Math.floor(update.length / 2.0), 1);
		sum += middleNumber;
	}
...
const test2 = (input) => {
	...
	for (let f = 0; f < incorrectUpdates.length; f++) {
		let update = [...incorrectUpdates[f]];
