...
const test = (input, allowMultipleRoutes) => {
	...
	while (trailsInProgress.length) {
		...
		trailsInProgress.forEach((trail) => {
			...
			// up
			const stepUp = y > 0 ? Number.parseInt(input[y - 1].charAt(x)) : -1;
			if (stepUp === step + 1) {
				const nextTrail = {
					trailhead,
					step: stepUp,
			...
			// right
			const stepRight =
				x < xMax - 1 ? Number.parseInt(input[y].charAt(x + 1)) : -1;
			if (stepRight === step + 1) {
				const nextTrail = {
					trailhead,
					step: stepRight,
			...
			// down
			const stepDown =
				y < yMax - 1 ? Number.parseInt(input[y + 1].charAt(x)) : -1;
			if (stepDown === step + 1) {
				const nextTrail = {
					trailhead,
					step: stepDown,
			...
			// left
			const stepLeft = x > 0 ? Number.parseInt(input[y].charAt(x - 1)) : -1;
			if (stepLeft === step + 1) {
				const nextTrail = {
					trailhead,
					step: stepLeft,
	...
	for (let i = 0; i < trailheads.length; i++) {
		...
		if (allowMultipleRoutes) {
			trailScore = trailsCompleted.filter(
				(tc) => tc.trailhead.join("-") === trailhead.join("-"),
			).length;
		} else {
			trailScore = [
				...new Set(
					trailsCompleted
						.filter((t) => t.trailhead.join("-") === trailhead.join("-"))
						.map((t) => t.path[t.path.length - 1].join("-")),
				),
			].length;
		}
		total += trailScore;
