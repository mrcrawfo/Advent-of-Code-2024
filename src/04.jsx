...
const test1 = (input) => {
	...
	const testFromX = (tx, ty, dx, dy) => {
		try {
			if (input[ty + dy].charAt(tx + dx) === 'M' && input[ty + 2*dy].charAt(tx + 2*dx) === 'A' && input[ty + 3*dy].charAt(tx + 3*dx) === 'S') {
				return 1;
			}
		} catch (e) {
			return 0;
...
const test2 = (input) => {
	...
	const testFromA = (tx, ty) => {
		if (tx > 0 && tx < input[ty].length - 1 && ty > 0 && ty < input.length - 1) {
			// up-right
			let da1 = input[ty - 1].charAt(tx + 1);
			// down-right
			let db1 = input[ty + 1].charAt(tx + 1);
			// down-left
			let da2 = input[ty + 1].charAt(tx - 1);
			// up-left
			let db2 = input[ty - 1].charAt(tx - 1);
			if (((da1 === 'M' && da2 === 'S') || (da1 === 'S' && da2 === 'M')) && ((db1 === 'M' && db2 === 'S') || (db1 === 'S' && db2 === 'M'))) {
				return 1
			}
		} else {
			return 0;
